...
class EncoderText(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, non_pad_mask, vision_feat, vision_mask, gt_labels=None, vision_labels=None, MLM=False, istest=False):
        ...
        if MLM:
            if self.cm:
                text_global_neg1, img_global_neg1 = self.sample_negative(vision_feat,text_output,non_pad_mask,vision_mask,head_mask,1)
                text_global_neg2, img_global_neg2 = self.sample_negative(vision_feat,text_output,non_pad_mask,vision_mask,head_mask,2)
                text_global_neg3, img_global_neg3 = self.sample_negative(vision_feat,text_output,non_pad_mask,vision_mask,head_mask,3)
...
class SCAN(object):
    ...
    def forward_embMLM(self, images, captions,  target_mask, vision_mask, gt_labels,vision_labels, volatile=False, istest = False):
        """Compute the image and caption embeddings
        """
        # Set mini-batch dataset
        images = Variable(images.float(), volatile=volatile)
        ...
        video_non_pad_mask = get_non_pad_mask(vision_mask).cuda().squeeze()
        loss,corr,total,corr_vis,total_vis = self.txt_enc(captions, token_type_ids, attention_mask,images,video_non_pad_mask,gt_labels,vision_labels,MLM=True)
